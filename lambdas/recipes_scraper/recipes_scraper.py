...
# Input JSON object just requires an "url" key
...
def process_message(message):
    ...
    if scraped_json is None:
        ...
        print(f"Scraped data retrieved: scraped_json: {scraped_json}")
    ...
    try:
        ...
        print(f"FINAL message_body: {message_body}")
    except Exception as e:
...
def recipes_scraper(event, context):
    ...
    # if json list is empty, return the failed message IDs to SQS
    if not json_list:
        print(f"No JSON objects to process, exiting...")
        ...
        print(f"Json list not empty: {json_list}")
    ...
    # if dataframe has no rows, return the failed message IDs to SQS
    if len(df) < 1:
        print(f"No rows in dataframe, exiting...")
