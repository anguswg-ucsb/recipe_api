...
from app.api.api_v1.endpoints.crud import _query_suggested_ingredients_trgm
from app.api.db import get_db_conn

router = APIRouter()

@router.get("/")
def get_suggested_ingredients_trgm(
    search: Union[str, None] = Query(default=None),
    limit: int = 10
):
    """
    Get suggested ingredients based on a search query using pg_trgm.
    ...
    try:
        ...
        ingredients = _query_suggested_ingredients_trgm(
